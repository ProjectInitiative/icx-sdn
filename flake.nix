  ...
  outputs = { self, nixpkgs, flake-utils, uv2nix, pyproject-nix, pyproject-build-systems, ops-utils }:
    let
      ...
      mkPkg = system:
        let
          ...
          docker = pkgs.dockerTools.buildImage {
            ...
            copyToRoot = pkgs.buildEnv {
              name = "image-root";
              paths = [ pkg pkgs.net-snmp pkgs.bash pkgs.coreutils ];
        ...
        in
        {
          inherit pkgs lib venv pkg docker ops opsApps;
        };

    in
    {
      packages = forSystems (system:
        let m = mkPkg system; in
        {
          default = m.pkg;
          docker = m.docker;
      ...
      apps = forSystems (system:
        let m = mkPkg system; in
        {
          ...
          build-docker = {
            type = "app";
            program = toString (m.pkgs.writeShellScript "build-docker" ''
              set -e
      ...
      devShells = forSystems (system:
        let m = mkPkg system; in
        m.pkgs.mkShell {
          packages = [ m.pkg m.pkgs.uv m.pkgs.net-snmp m.pkgs.nixfmt-rfc-style ];
